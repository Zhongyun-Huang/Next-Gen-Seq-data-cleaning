...
inlist_fh.each do |line| 
  ...
  hmp_fh.each do |line| 
  	...
  	snp_name = raw[ 0 ]
  	puts "72: raw.size: #{raw.size} for #{snp_name}" 																			#temp
  	snp_pos = raw[ 3 ].to_i
  	...
  	raw.slice!(0..10)
  	puts "81: raw.size: #{raw.size} for #{snp_name}" 																			#temp
  	if line =~ /^rs#/  # header line
  		if first_file == true
			...
			num_indivs = raw.size
			puts "100: num_inidivs: #{num_indivs} for #{snp_name}" 															#temp
			last_i = num_indivs - 1
			for i in 0..last_i
				ind_id = ind_ids[ i ]
				geno = raw[ i ]
...
phy_fh.puts "#{num_taxa}\s#{num_snps}" # 
# Print out the indivs and genos from all chromosomes to a single file
ind_ids.each do |ind_id|
	genos = big_struct[ ind_id ]
	puts "Based on the genos array, it looks like there are #{genos.size} SNPs." if ind_id == "1658" 		#temp
	puts "Based on the genos array, it looks like there are #{genos.size} SNPs." if ind_id == "1659" 		#temp
	...
	num_spaces = 10 - ind_id.length
	spaces = ""
	for i in 1..num_spaces do 
		spaces += " "
	end
	phy_fh.puts "#{ind_id}#{spaces}#{back_part_string}"
end																																							#temp
