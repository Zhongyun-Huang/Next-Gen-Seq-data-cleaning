...
def get_members( pm_fn, pop_codes )
	...
	pm_fh.each do |line| 
		...
		raw = line.chomp.split(/\t+/)  # tab defines field limits
		labname = raw[0].strip
		parts = labname.split(/\-/)  # I just want the middle part
		genomic_name = parts[ 1 ]
		gen_loc = raw[1]
		spec_loc = raw[2]
		#members[ genomic_name ] = spec_loc.strip   # allows later lookup of the spec group
		reg_members[ genomic_name ] = gen_loc.strip   # allows later lookup of the broader group
...
puts "line 132", reg_members.keys.size    																								#temp
...
ind_ids.each do |ind_id|
	...
	puts "#{ind_id}; #{reg_members}"if ind_id == "103830"  #temp
	...
	#puts members  																																					#temp
	#puts "id_code: #{id_code}; members[ id_code ]: #{members[ id_code ]}; loc_string: -#{loc_string}-"			#temp
