...
inlist_fh.each do |line| 
  ...
  hmp_fh.each do |line| 
  	...
  	# Get position from line
		...
		puts "raw.size: #{raw.size}, raw.size.class: #{raw.size.class}"
		raw.slice!(0..10)
...
out_handle.puts "nhl_ctr: #{nhl_ctr}"																											#temp
out_handle.puts "t_shoot_ctr: #{t_shoot_ctr}"																							#temp
out_handle.puts "lines_examined: #{lines_examined}"																				#temp
out_handle.puts "intermed_ctr: #{intermed_ctr}"																						#temp
out_handle.puts "q_drops: #{q_drops}"																						#temp
