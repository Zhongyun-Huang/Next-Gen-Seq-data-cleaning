...
# == Usage
# 
#   ./hap2struct.rb -i inlist.txt -o for_struct_080112n10000.stru -s subsets.txt -n 1000
#
# -s is the filename for a two-col file with indivs and groups.
#
...
fail "\nUsage: #$0  -i inlist.txt -o for_struct_080112n10000.stru -s subsets.txt -n 1000\n" if ARGV.size < 4
...
# Make a hash of hashes to store the individual names by group
group_codes = Hash.new
# Get subsets of individuals from file
subsets_fh = File.new( subsets_fn )
subsets_fh.each do |line| 
	next if line =~ /^\s*$/  # checks for blank line
	raw = line.chomp.split(/\s+/)  # space defines field limits
	indiv_id = raw[ 0 ]
	subset_id = raw[ 1 ].to_i
	# add indiv_id to array
 	# Not useful?    group_codes[ subset_id ] = indiv_id
 	group_codes[ indiv_id ] = subset_id
end
...
ind_ids.each do |ind_id|
	...
	geno, snp_name = doublet
		if 1 == 2  #changed this line to keep hets
		else
			back_part.push( geno )			  				
			kept_count += 1	if ind_id == "103830" 		#temp			
		end
	end
	grp = group_codes[ ind_id ]
